...
def load_data(orders_path: Path, items_path: Path, customers_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Carrega os dados de pedidos, itens e clients a partir do csv.
    Args:
    ...
    Returns:
        ...
        Items: Dataframe contendo os itens do pedido.
        Customers: Dataframe contendo os clientes.
    """

    orders = pd.read_csv(
        orders_path,
        parse_dates=["order_purchase_timestamp", "order_approved_at", "order_delivered_customer_date", "order_estimated_delivery_date"],
    )
